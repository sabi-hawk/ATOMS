...
Note:
 This security layer uses encasementlib.r2py, restrictions.default, repy.py
and Python
...
FUNC="func"
OBJC="objc"
# Repy V2 Library Reference: seattle.poly.edu/wiki/RepyV2API
class ABFile():
...
""")
    ...
    def writeat(self,data,offset):
        ...
        # cannot return exceptions during runtime
        try:
    ...
    def readat(self,bytes,offset):
        ...
        mycontext['lock'].acquire(True)
        try:
            contentRead = self.Afile.readat(bytes,offset)
            # release lock
            mycontext['lock'].release()
            return contentRead
        except LockDoubleReleaseError:
    ...
    def close(self):
        ...
        fileContent = list(readContent)
        mycontext['lock'].release()
        # check for first and last index of contents for requirement
        try:
            if (fileContent[0] == 'S' and fileContent[len(fileContent)-1]=='E'):
            self.Afile.close()
            removefile(self.Afn)
            self.Afile = openfile(self.Afn, True)
            mycontext['lock'].acquire(True)
            self.Afile.writeat(readContent, 0)
            mycontext['lock'].release()
        except LockDoubleReleaseError:
            pass
